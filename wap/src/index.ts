...
const MIN_FRICTION = MIN_GRAVITY / 2;
const MAX_FRICTION = MIN_GRAVITY;
const MIN_LIFE = 20;
const MAX_LIFE = 90;
...
function Friction<TBase extends ParticleConstructor>(Base: TBase) {
    ...
    class Friction extends Base {
        ...
        update(seconds:number) {
            const friction = (Friction.friction * (this.radius / 4));
            const ddy = Math.abs(this.dy) * (friction * seconds) * Math.sign(this.dy);
            const ddx = Math.abs(this.dx) * (friction * seconds) * Math.sign(this.dx);
            this.dy -= ddy;
            this.dx -= ddx;
            if(this.trace) {
